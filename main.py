...
from utils.video import VideoController
...
def process():
    ...
    inferrer = model.InferMedia(stdout)
    modelRes = []

    def worker(results):
        results = pipe.visualize_movie(
            loadedVideo,
    ...
    modelThread = threading.Thread(target=worker, name="modelThread", args=[modelRes])
    modelThread.start()
...
def terminalOutput(text, clearText=True, error=False, command=False, wipe=False):
    if clearText:
        commandInput.delete("1.0", "end")

    if wipe:
        terminalText.config(state="normal")
        terminalText.delete("1.0", "end")
    ...
    if text:
        ...
        with open("logfile.txt", "a") as f:
            if error:
                ...
                terminalText.insert("end", "✖", "symbol-err")
...
if __name__ == "__main__":
    ...
    # Add TkinterVideo to videoFrame & Initialize controller
    videoCanvas = Canvas(videoFrame)
    videoCanvas.grid(row=0, column=0, sticky="nsew")
    videoController = VideoController(app, videoCanvas, terminalOutput)
