...
def convert_video_to_frames(cv2_capture, frame_rate, crop_video_length, output, return_consistent_length=False,
                            print_flag=True):
    """
    ...
    frames = convert_video_to_x_fps(cv2_capture, frame_rate, output=output, print_flag=print_flag)

    frame_rate_crop_video_length = round(
        frame_rate * crop_video_length)  # Target length of each clip expressed in frames
    ...
    if print_flag:
        output(f"Original Clip Length: {len(frames)}")
        output(f"Frame Count: {len(frames)}")
        len_of_clips = [len(frames_clips[i]) for i in range(len(frames_clips))]
        output(f"Small Clip Length: {min(len_of_clips)} at index {len_of_clips.index(min(len_of_clips))}")
        output(f"Large Clip Length: {max(len_of_clips)} at index {len_of_clips.index(max(len_of_clips))}")
